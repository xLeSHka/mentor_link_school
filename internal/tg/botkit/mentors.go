...
func MentorsKeyboard(bot *Bot, userID, groupID uuid.UUID, page, size int) (tgbotapi.InlineKeyboardMarkup, error) {
	students, total, err := bot.StudentService.GetMyMentors(context.Background(), userID, groupID, page, size)
	if err != nil {
	...
	for _, student := range students {
		rows = append(rows,
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData(student.Mentor.Name, student.Mentor.ID.String()),
			),
		)
	}
...
func Mentors(stack CallStack) CallStack {
	//return Chop(stack)              // delete or comment out after finishing work
	stack.Action = Mentors // Set self as current Action
	data := userDatas[stack.ChatID]
	if stack.IsPrint {
		stack.IsPrint = false
		if data.LastMes != -1 {
			keyboard, err := MentorsKeyboard(stack.Bot, data.User.ID, data.Group.ID, data.Page, data.Size)
			if err != nil {
				data.LastMes = -1
				data.Profile = nil
			...
			stack.IsPrint = false
			text := fmt.Sprintf("%s\n\n%s", MentorsMenuTemplate, MentorsMenuTextTemplate())
